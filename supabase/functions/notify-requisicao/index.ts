...
Deno.serve(async (req) => {
  try {
    ...
    // Suporte tanto para webhook direto do Supabase quanto chamada manual
    ...
    // Precisamos garantir que apenas envie se o status for aprovado/reprovado
    if (record.status !== 'aprovado' && record.status !== 'reprovado') {
      return new Response(
        JSON.stringify({ message: 'Ignorado. Status não é aprovado nem reprovado.' }),
        {
          status: 200,
          headers: { 'Content-Type': 'application/json' },
        },
    ...
    // Ignorar se o status não mudou (caso old_record esteja disponível)
    if (old_record && old_record.status === record.status) {
      return new Response(JSON.stringify({ message: 'Ignorado. Status não foi alterado.' }), {
        status: 200,
        headers: { 'Content-Type': 'application/json' },
      })
    }
    ...
    if (isAprovado) {
      conteudoAdicional = `
        <div style="margin-top: 16px; padding: 12px; background-color: #f3f4f6; border-radius: 6px; text-align: center;">
          <span style="display: inline-block; vertical-align: middle; color: ${corPrincipal}; font-weight: bold;">
            ✅ Processando
          </span>
        </div>
      `
      ...
      conteudoAdicional = `
        <div style="margin-top: 16px; padding: 12px; background-color: #fee2e2; border-radius: 6px; text-align: center;">
          <span style="display: inline-block; vertical-align: middle; color: #b91c1c; font-weight: bold;">
      ...
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background-color: ${corFundo}; border-radius: 8px; overflow: hidden; border: 1px solid #e5e7eb;">
        ...
        <div style="padding: 32px; color: ${corTexto};">
          ...
          <div style="text-align: center; margin-top: 32px; margin-bottom: 16px;">
            <a href="${linkAcao}" style="background-color: ${corDestaque}; color: ${corPrincipal}; text-decoration: none; padding: 12px 24px; border-radius: 6px; font-weight: bold; display: inline-block;">${textoBotao}</a>
          </div>
        </div>
