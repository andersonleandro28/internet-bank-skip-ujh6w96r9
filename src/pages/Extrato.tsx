...
import { Search, Filter, Download, ArrowUpRight, ChevronRight } from 'lucide-react'
...
export default function Extrato() {
  ...
  const grouped = requisicoes.reduce((acc, req) => {
    const d = new Date(req.created_at).toLocaleDateString('pt-BR')
    if (!acc[d]) acc[d] = []
    acc[d].push({
      id: req.id,
      title: req.tipo,
      ...
      time: new Date(req.created_at).toLocaleTimeString('pt-BR').slice(0, 5),
      icon: ArrowUpRight,
  ...
  const transactionGroups = Object.keys(grouped).map((k) => ({ date: k, items: grouped[k] }))
                ...
                {group.items.map((tx: any) => (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-primary capitalize">{tx.title}</p>
